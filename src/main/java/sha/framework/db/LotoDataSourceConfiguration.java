...
@Configuration
@MapperScan(basePackages = DBConfig.LOTO_MAPPER_PACKAGE, sqlSessionTemplateRef = DBConfig.LOTO_SQLSESSION_BEAN_NAME)
public class LotoDataSourceConfiguration {
	@Autowired
	private LotoProperties db2Properties;
	
	@Autowired
	private ApplicationContext context;

 
	@Bean(name = { DBConfig.LOTO_SOURCE_BEAN_NAME })
	@ConfigurationProperties(prefix = DBConfig.LOTO_YML_SOURCE_NAME)
	public DataSource dataSource() {
		return db2Properties.getDataSource();
	}
	
	@Bean(name = DBConfig.LOTO_SQLSESSION_BEAN_NAME)
	public SqlSessionTemplate sqlSessionTemplate(@Qualifier(DBConfig.LOTO_SOURCE_BEAN_NAME) DataSource dataSource) throws Exception {
		...
		bean.setMapperLocations(resolver.getResources(DBConfig.LOTO_MAPPER_XML_PATH));
		return new SqlSessionTemplate(bean.getObject());
	}
 
	@Bean(name = "db2DataSourceInitializer")
	public DataSourceInitializer dataSourceInitializer(@Qualifier(DBConfig.LOTO_SOURCE_BEAN_NAME) DataSource dataSource) {
